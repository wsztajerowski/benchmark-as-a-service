...
show_help() {
    echo "Usage: $0 --mongo-connection-string <connection-string> --aws-profile <aws-profile>"
    echo ""
    echo "Options:"
    echo "  --mongo-connection-string   MongoDB connection string (default: mongodb://localhost:27017/local_test)"
    echo "  --aws-profile               AWS CLI profile to use (required)"
...
echo "Step 1: Validating required tools..."
./validate-tools.sh || exit 1

# Step 2: Prepare test data (run Maven command)
echo "Step 2: Preparing test data..."
echo "2.1: Run Maven build on project"
...
REQUEST_ID=$RANDOM
echo "Step 3: Running act command..."
...
if [ "${PIPESTATUS[0]}" -ne 0 ]; then
    echo "Error: act command failed."
    exit 1
fi

# Step 4: Run assertions
echo "Step 4: Running S3 verification..."
echo "Verifying if async output and flamegraph exist on S3..."
./verify-s3.sh --profile "$AWS_PROFILE" --s3-bucket "$S3_BUCKET" --key "${REQUEST_ID}/jmh-with-async/output.txt" --check-size || exit 1
./verify-s3.sh --profile "$AWS_PROFILE" --s3-bucket "$S3_BUCKET" \
...
echo "Step 4: Running MongoDB verification..."
echo "Verifying if document with benchmark results exists in MongoDB..."
./verify-mongo.sh --connection-string "$MONGO_CONNECTION_STRING" --collection "jmh-benchmarks" --key "_id.requestId" --value "$REQUEST_ID" || exit 1
