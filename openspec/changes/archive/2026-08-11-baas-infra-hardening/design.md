...
- **[Risk] `DeletionPolicy: Retain` means `baas admin teardown` leaves an orphaned bucket that a later `baas admin setup` cannot recreate under the same name** → Mitigation: `--delete-bucket` empties the bucket *and* calls `DeleteBucket` explicitly, because `Retain` means CloudFormation never removes it. On the default (retain) path teardown prints the bucket name and states plainly that the next `baas admin setup` will fail until it is deleted — the prefix is a hash of the caller ARN, so the name is deterministic and the collision is certain, not hypothetical.
